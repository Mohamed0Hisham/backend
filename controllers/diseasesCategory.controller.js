...
export const show = async (req, res, next) => {
  try {
    ...
    const diseasesCategory = await DiseasesCategory.findById(id);
    if (!diseasesCategory) {
      return next(errorHandler(404, "Can't find this Disease Category"));
    }

    const diseases = await DISEASES.find({ diseasecategoryId: id });

    return res.status(200).json({
      data: {
        category: diseasesCategory,
        diseases: diseases,
      },
