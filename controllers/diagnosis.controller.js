  ...
  try {
    ...
    const diagnoses = await Diagnosis.find({ patient: patientId })
      .populate({
        path: "doctor",
        select:
          "-password -createdAt -updatedAt -role -appoints -isVerified -__v",
      })
      .populate("medications")
      .lean();
    if (!diagnoses || diagnoses.length === 0)
      return next(errorHandler(404, "No diagnoses found for this patient"));
...
export const fetchSpecificDiagnosis = async (req, res, next) => {
  try {
    ...
    const diagnosis = await Diagnosis.findById(diagnosisId)
      ...
      .lean();
    if (!diagnosis)
      return next(errorHandler(404, "No diagnosis found for this patient"));
...
export const addDiagnosis = async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(symptoms))
      return next(errorHandler(400, "provide symptoms as an array of strings"));
...
export const updateDiagnosis = async (req, res, next) => {
  try {
    ...
    if (!diagnosis) return next(errorHandler(404, "Diagnosis doesn't exist"));
    ...
    if (
      ...
      return next(errorHandler(400, "at least one field should be updated"));

    const updated = await Diagnosis.findByIdAndUpdate(diagnosisId, req.body, {
      new: true,
    })
      .populate({
        path: "doctor",
