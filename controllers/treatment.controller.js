...
export const index = async (req, res, next) => {
  try {
    ...
    const treatments = await TREATMENT.find().skip(skip).limit(limit).lean();
...
export const store = async (req, res, next) => {
  ...
  try {
    // 1. Extract new parameters from req.body
    const {
      diseaseID,
      name,
      description,
    ...
    if (!diseaseID || !name || !description || !dosage || !instructions) {
    ...
    // 3. Create new treatment with all parameters
    const newTreatment = new TREATMENT({
      ...
      instructions,
      quantity,
      refills: refills ?? 0, // default to 0 if not provided
      notes,
    });
